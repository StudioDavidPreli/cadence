...
describe('transformPoint', () => {
  const at = (over) => ({ x: 0, y: 0, rotation: 0, scale: 1, ...over })

  it('translates', () => {
    expect(transformPoint({ x: 1, y: 2 }, at({ x: 10, y: 20 }))).toEqual({ x: 11, y: 22 })
  })

  it('scales about the attachment point, not the origin', () => {
    const p = transformPoint({ x: 2, y: 0 }, at({ x: 100, y: 100, scale: 3 }))
    expect(p.x).toBeCloseTo(106)
    expect(p.y).toBeCloseTo(100)
  })

  it('rotates', () => {
    const p = transformPoint({ x: 1, y: 0 }, at({ rotation: Math.PI / 2 }))
    expect(p.x).toBeCloseTo(0)
    expect(p.y).toBeCloseTo(1)
  })

  it('rotates then scales then translates, in that order', () => {
    const p = transformPoint({ x: 1, y: 0 }, at({ x: 5, y: 5, rotation: Math.PI, scale: 2 }))
    expect(p.x).toBeCloseTo(3)
    expect(p.y).toBeCloseTo(5)
  })

  it('leaves the attachment point itself fixed under any rotation', () => {
    // The origin convention: a mark is centered on its attachment point, so
    // that point must not move when the stamp turns.
    for (const rotation of [0, 0.4, Math.PI / 2, 2.9]) {
      const p = transformPoint({ x: 0, y: 0 }, at({ x: 40, y: 60, rotation, scale: 0.2 }))
      expect(p.x).toBeCloseTo(40)
      expect(p.y).toBeCloseTo(60)
    }
  })

  it('preserves distance ratios, so a mark cannot shear', () => {
    const a = transformPoint({ x: 0, y: 0 }, at({ rotation: 1.1, scale: 0.3 }))
    const b = transformPoint({ x: 10, y: 0 }, at({ rotation: 1.1, scale: 0.3 }))
    const c = transformPoint({ x: 0, y: 10 }, at({ rotation: 1.1, scale: 0.3 }))
    expect(Math.hypot(b.x - a.x, b.y - a.y)).toBeCloseTo(3)
    expect(Math.hypot(c.x - a.x, c.y - a.y)).toBeCloseTo(3)
  })
})

describe('composeStamps', () => {
  const library = [
    { strokes: [{ pts: [{ x: -1, y: 0 }, { x: 1, y: 0 }], color: '#aaaaaa' }] },
    {
      strokes: [
        { pts: [{ x: 0, y: -1 }, { x: 0, y: 1 }], color: null, tokenBound: true },
        { pts: [{ x: -1, y: -1 }, { x: 1, y: 1 }], color: '#bbbbbb' },
      ],
    },
  ]
  const placement = { ix: 0, iy: 0, k: 0, x: 50, y: 60, rotation: 0, scale: 2, markIndex: 0 }

  it('places a mark into world space', () => {
    const { stamps } = composeStamps([placement], library)
    expect(stamps).toHaveLength(1)
    expect(stamps[0].strokes[0].pts).toEqual([{ x: 48, y: 60 }, { x: 52, y: 60 }])
  })

  it('carries per-stroke color through untouched', () => {
    // Resolving color to a theme value is the renderer's job, once, ahead of
    // aggregation, for both faces. compose must not pre-empt it.
    const { stamps } = composeStamps([{ ...placement, markIndex: 1 }], library)
    expect(stamps[0].strokes.map((s) => s.color)).toEqual([null, '#bbbbbb'])
    expect(stamps[0].strokes.map((s) => s.tokenBound)).toEqual([true, false])
  })

  it('keeps the placement fields on the stamp', () => {
    const { stamps } = composeStamps([placement], library)
    expect(stamps[0]).toMatchObject({ ix: 0, iy: 0, k: 0, x: 50, y: 60, markIndex: 0 })
  })

  it('skips a placement whose mark is missing rather than throwing', () => {
    const { stamps } = composeStamps([{ ...placement, markIndex: 99 }], library)
    expect(stamps).toEqual([])
  })

  it('counts strokes', () => {
    const { stats } = composeStamps([placement, { ...placement, markIndex: 1 }], library)
    expect(stats.stamps).toBe(2)
    expect(stats.strokes).toBe(3)
  })

  it('is deterministic', () => {
    const a = composeStamps([placement], library)
    const b = composeStamps([placement], library)
    expect(a.stamps).toEqual(b.stamps)
  })
})

describe('strokesOf', () => {
  it('flattens a display list into the array raster.aggregate wants', () => {
    const stamps = [
      { strokes: [{ pts: [], color: 'a' }, { pts: [], color: 'b' }] },
      { strokes: [{ pts: [], color: 'c' }] },
    ]
    expect(strokesOf(stamps).map((s) => s.color)).toEqual(['a', 'b', 'c'])
  })
