...
A spring can also stand in for `ease.overshoot` on the components that carry a `motionMode` prop (Button, Card, Toggle). The prop defaults to `'bezier'`, so nothing ships rewired; Token Lab's per-demo spring toggle (a single coil icon in the demo's label row, left of the `</>` button) is the only caller that passes `'spring'`, and it flips the demo's transition to `{ type: 'spring', ...tokens.spring }` in place. It is a comparison affordance, the imitation next to the physics on one component, not a change to shipped motion.
