...
2. `toDtcgJson(state)` serializes that object to the W3C Design Tokens Community Group format, wrapping each leaf in `$type` / `$value` under a top-level `motion` namespace. This is the shape Style Dictionary, Tokens Studio, and Figma Variables consume. It targets the Design Tokens Format Module 2025.10, the first stable release (28 October 2025), which requires a duration `$value` to be an object, `{ "value": 200, "unit": "ms" }`, rather than the `"200ms"` string earlier drafts allowed (moved 2026-09-13; see `docs/decisions/dtcg-2025-10-conformance-2026-09-13.md`). Cadence always writes `ms`, so every artifact it ships reads in one unit. The spec has no motion-specific delay type, so delays serialize as `duration`. It also has no spring type, so the three spring params serialize as plain `number` leaves (the same `$type` scale uses) under a `spring` group. A spring is not one composite value here, it is three unitless numbers, and the group name carries the "these compose one spring" meaning. No invented type, and it round-trips through `importTokens`.
...
4. `toCssVars(state)` serializes the same object to a `:root` block of the editable `--motion-*` custom properties, in the exact variable names and units used by `src/tokens/motion.css`. It is a drop-in replacement for that block. Only the editable token scale is emitted; the `--feedback-*` chrome timings in `motion.css` are not design tokens and are left out. Since 2026-09-13 it also appends a `prefers-reduced-motion` block carrying the resolution the provider applies (every duration 10ms, every delay 0; easing, scale, spring and the scalar deliberately untouched), because the provider does not travel with an exported file and a downstream system would otherwise have to invent an answer. `motion.css` itself does NOT carry that block: the site answers reduced motion in the provider and Token Lab opts out of it on purpose, so a media block in the site's own stylesheet would collapse the demos underneath that opt-out. The same resolution ships as `dist/cadence.resolver.json`, a Design Tokens Resolver Module 2025.10 document beside the token file. Both read one constant, `REDUCED_MOTION_RESOLUTION`; see `docs/decisions/reduced-motion-resolution-2026-09-13.md`.
...
Import is the inverse of export, plus validation. `importTokens(text)` in `packages/tokens/src/index.js` parses a file, detects the format (DTCG or flat), and returns a discriminated result. It never throws to the caller: internal helpers throw a named `ImportError` for fatal cases, caught at the boundary and returned as `{ ok: false, error }`. A successful import returns `{ ok: true, state, report }`. A DTCG duration leaf is read in all three shapes it has ever taken: the 2025.10 object (a `"s"` unit converts to ms), the pre-2025.10 `"200ms"` string, and a bare number, so no file the tool has ever written stops loading. The object form is accepted only on the two duration-typed families; on a `number` leaf it is a malformed file and throws.
