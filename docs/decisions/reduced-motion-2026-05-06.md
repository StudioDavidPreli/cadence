...
**Rive surface pass (2026-07-17, the session after the gate landed).** The policy: chrome Rive freezes at a rest state; demonstration Rive starts paused behind an explicit play affordance. The code half is done:

- **The Motion Tiles field starts paused** under the OS preference, at its rest state (progress 0, blessed by David's Tier 3 r1c6 scrub). The existing Pause/Play control is the affordance, with the same `pausedChoice ?? prefersReduced` override pattern as the library's icon pause. The pause reaches clawd tiles too, through the existing `paused` prop path.
- **The three unhandled chrome buttons freeze**: the grid's waving logo (`ClawdLogoButton`), the grid's Problems button, and the home bug-report button. Each already deferred `rive.play` until its instance bound; under reduce the effect pauses instead, holding the bound first frame. Hover and press responses freeze with the state machine; clicks are owned by the wrapping `<button>` elements and keep working.
- **Already correct before this pass** (each gates `autoplay` on `useReducedMotion` and holds a first frame): the desktop hero, the mobile-gate hero, the Studio logo link, the Enter button, and the landing's Motion Tiles logo. `MotionTilesTitle` is a static SVG and was never motion.

~~What remains is authoring, not code, in David's Rive editor: designed rest poses where a held first frame reads badly (the desktop hero's "stopped dead" read, the mobile-gate pair) and the Enter button's one-theme legibility issue at rest, plus his eye over every newly frozen first frame.~~ Token Lab's Carousel demo is deliberately untouched: it sits inside the tool's documented opt-out scope.

**Poster update (2026-07-18): the authoring landed as static SVGs, not Rive rest poses.** David's call, and the better architecture: some files draw nothing until their state machine's clock advances (the desktop hero), so a paused canvas can never be a designed still. Six surfaces now render a per-theme SVG poster under the OS preference instead of mounting Rive at all: the desktop hero, the mobile-gate hero, the Token Lab title, the Motion Tiles overview title, the Enter button, and the home bug-report button (upgrading its 2026-07-17 held-frame freeze). The files live in `/public/fallBacks` with flat `<surface><Theme>.svg` naming; `riveFallbackSrc()` (`src/utils/riveFallbacks.js`) maps theme to file. Each component split its Rive hooks into an inner component so the poster branch never fetches the `.riv`, and on the home page under reduce-motion no `.riv` and no WASM binary loads at all (asserted by a Tier 1 network guard). Buttons that sized themselves from `rive.bounds` read the SVG's natural size instead. The same session homogenized `heromobile.riv` to the four-instance contract (see the mobile-gate doc's addendum), retiring the two-contract landmine. Still on held first frames rather than posters, acceptable by David's sweep: the grid's logo and Problems buttons. (The Studio logo link joined the poster set the same evening, once its four `singleLineLogo<Theme>.svg` exports landed; the mobile gate is now entirely Rive-free under the preference.)
