...
**Run by David, same day, on production.** All fourteen links clicked, two of them (`rive` to the tool, `claudeai` to the case study) clicked twice on purpose so the readout would show variance: a uniform field of ones cannot distinguish a counter that increments from a row-per-link fluke. The SQL query returned all fourteen rows with the variance matching the clicks. The pipeline is closed; what remains of launch is content, not infrastructure. (Same session: `CASE_STUDY_URL` gained its trailing slash, sparing every case-study click the host's 308 hop.)
