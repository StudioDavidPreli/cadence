...
- ~~**Unify the preset vocabularies.**~~ **Closed 2026-07-16, David's decision: the vocabularies stay separate, on purpose.** The two tools describe different motion classes. Token Lab's tokens describe event-driven interaction motion: a press, an enter, an exit, each wanting a duration and a bezier. The grid describes an ambient looping field: a period, an envelope exponent, a spread, cell and gap geometry. The values were tuned per context (Token Lab's Cinematic is 4x its Snappy on `duration.slower`; the grid's is 1.6x, because an ambient field that swings 4x goes soporific), and the user works one tool bar at a time. Wiring the tools together would move sliders in a section nobody is looking at, to serve a demo moment. What the system shares across the two motion classes is the vocabulary of names: the same three personalities, Snappy, Standard, Cinematic, completed the same day by renaming Token Lab's Default preset to Standard. What it scopes per class is the interpretation. Knowing where a vocabulary ends is part of designing one; that is the position, and David is prepared to defend it. CLAUDE.md's Motion Tiles pitch and the case study's One Vocabulary at Scale section were rewritten the same day to claim exactly this and no more. Original item kept for the record: Motion Tiles carries its own Snappy, Standard, Cinematic presets with their own speed, easing, and spread; Token Lab has its own presets; bridging them was floated as a case-study thread.
...
**Not done, still open.** *(Update 2026-07-16, later: all three Future work items are now closed. The grid-chunk prefetch and the Principles retrofit shipped; the preset-vocabulary unification closed as a scoping decision, recorded in the Future work bullet above.)* The `Future work` items above stand: prefetch the grid chunk on hover, unify the two preset vocabularies, retrofit Principles to lazy. One small UX option noted in build: the Enter button has no hover cue beyond the cursor and the loop, on the assumption the Rive may carry hover; a CSS hover can be added if wanted. *(The Enter-hover question was settled by David's Tier 3 sweep, 2026-07-16: the Rive carries the hover response.)*
