...
`duration.slow` (Stepper beat 1, the checkmark, and beat 2, the connector fill; also the ProgressBar fill), `duration.fast` (the step number swapping out), `ease.enter` (the checkmark and the number arriving), `ease.standard` (the connector fill, and the ProgressBar's forward curve), `delay.short` (the gap between beat 1 and beat 2). Beat 3's `delay.medium`, `ease.enter` and `ease.standard` are still read in compact mode but drive the label and description, which the card hides; `delay.long` reaches only the completion overlay, which compact does not mount. The Stepper reads `duration.base` nowhere (corrected 2026-09-15; the line had said `base` since the card was built).
