...
What remains is elective, session-sized work, none of it production risk: ~~the **Principles lazy retrofit**~~ (done 2026-07-16, the session after this close; see the Part 1 row and the rive-scaling doc's addendum), the physics-spring token family, ~~the `.riv` asset-prefetch half~~ (done 2026-07-16, same evening: `RIV_PREFETCH_MANIFEST` exported from the grid, fetched by the landing; verified on built output, see the rive-scaling doc's second addendum), the preset vocabulary unification, the automated Tier 1 suite, the CSS import asymmetry, the reserved deep-link route, ~~an ESLint baseline~~ (adopted 2026-07-16, the session after the churn hold; see the Part 2 bullet), ~~the `claude-workflow.md` stub~~ (written 2026-07-16, the session after this close; see the Part 2 row), and the conditional git-history purge if the repo ever goes public. On the case study: David's What I Learned rewrite, the hiring-manager edit, portfolio integration, and the sharing pass. This document is now a record, not a to-do list; the tracker's Future work section carries what stays alive.
...
**Editable-vs-fixed explainer close (2026-07-16, the session after this close):** the in-tool explainer was already done: the 2026-06-22 Token Lab guide absorbed it (the code-view section and FAQ question one), and both read correct against current source, so the item closes as record-keeping. What the verification exposed is that the docs the FAQ points at did not survive the 2026-07-08 spring → overshoot rename (commit a322f48). Fixed this session: `token-architecture.md` still described three fixed tokens and three easing slots (now two fixed, four slots, the Explore-only overshoot nuance, and a dated note on the rename); the case study named the dead `ease.spring` seven times (renamed, and P10's duration corrected fast → slow against the shipped badge); the Exaggeration companion doc described a pre-keyframe badge (rewritten against the component); the Appeal and Follow Through companion docs carried the dead name (renamed, and Follow Through's duration corrected base → slow per the Carousel's deliberate read); a stale `resolveToken.js` comment still called overshoot a fixed reference. ~~**Still open, David's call per principle:**~~ **Verdicts landed same day, all three closed.** Three demos contradicted their own descriptive layer beyond the name. **P01 (David: the scale is both the squash and the stretch, emphasis supplied by the easing tokens):** the Button's release had silently ridden Framer's default spring, an easing no token owned; the release now reads `ease.overshoot` explicitly (one added `transition` prop), the card string appends `ease.standard · ease.overshoot`, and the code-view snippet, `TOKEN_COMPONENT_MAP`, companion doc, and case-study note all follow. **P02 (copy follows code):** every layer rewritten to the shipped Drawer, `duration.slow` on `ease.enter`/`ease.exit` keyframes with the countermotion on the exit: card string and summary, the copy inside the drawer, `anticipation.md`, and both case-study passages. **Carousel:** card string corrected to `duration.slow`. The Drawer code-view snippet's stale `-4%` keyframes were trued to the shipped `-10%` in passing. Verified on built output; David feel-checked the full diff same day and passed it, including the Button release, the one change that alters shipped motion (bezier overshoot where the default spring used to be). Committed and pushed; this session is closed.
...
**Second checklist session (2026-07-16, evening):** thirteen of the fourteen rows the item 4 run left open are now closed on built output, zero new failures. Playwright rows: guide-on-open and Overview return, both deep links plus back-button traversal, hover-vs-pressed distinctness (with the HC outline mechanism read from the Button source), the out-of-range import (every clamp itemized in the report modal, nothing silent), Constrained-vs-Explore (expand, drive an Explore-only value, reset-to-Default on toggle-off per the documented model), and the full OS-preference first-load matrix including stored-choice-wins with no flash. Code-audit rows: disabled states (N/A for demos; the two real disabled controls style `:disabled`), hover-never-sole-carrier, loading and empty states, the 172-rule background-color/color sweep, and the every-animated-value-traces row (scoped: the gate plus a clean sweep of unscanned surfaces; TokenFidelity's literal and the Motion Tiles preset scalars are the documented exceptions). The one row left open is accent-never-decorative: the contrast half passed by scripted ratios (accent as text clears 4.5:1 everywhere, worst 5.25:1), but the census found the "strokes-only, never text" claim in CLAUDE.md and the contrast audit had drifted (seven text uses, six of them state signals), both corrected same day, and the PrincipleCard category chips' taxonomy use of the accent family is a semantic call only David can make. **Verdict landed same session:** David re-specified the chips per theme (dark tinted-text on the brand hues, light solid fills with near-black text, HC monochrome outline/inversion pair); they moved onto their own `--color-chip-*` tokens and off the accent family, the row is ticked, and the spec with ratios lives in the contrast audit's correction section. What remains under Critical flag 2 after this session: the automated Tier 1 suite, the per-principle Tier 2/3 matrix, and extending the two documents' row sets to cover Motion Tiles as a first-class surface.

**Easy-wins close (2026-07-16, night, the session that ran the second checklist pass): the audit's elective shortlist is spent.** Six moves, five commits, all verified on built output where there was output to verify. The chip verdict landed and shipped: David re-specified the category chips per theme (light and dark share the brand hues, light as solid fills with near-black text; both HC themes monochrome, classic outline and extended inversion), they moved onto their own `--color-chip-*` tokens, and the deploy checklist closed its last row at 56 of 56. The `.riv` prefetch shipped as the second half of the Rive-scaling item: the grid exports `RIV_PREFETCH_MANIFEST`, the landing warms all 20 files during the idle read, and Enter serves entirely from the primed cache (3ms 304s, zero full downloads). The decision round closed all four parked design questions: M2 confirmed, both Modal-era token candidates declined, HC-light keeps `#855a0d` (the AAA hex had become accent3, so AAA would have cost the active/warning distinction), and the nav stays in the middle; each verdict is recorded in the doc that parked its question. The micro tidy resolved the Blockers placeholder and the harmonized doc's nested-vs-flat scale naming. The ESLint churn is quantified and held (the bullet in Part 2 carries the numbers and the adoption shape). One session note: a Playwright `goto` that differs only in hash does not renavigate, which had two measurement passes silently reading a stale document; force a real navigation through `about:blank` before trusting built-output resource timings. What the project still owes is only the substantive queue: the automated Tier 1 suite, the checklist's Motion Tiles row extension, the preset vocabulary unification, the physics-spring family, the P14-P16 reduced-motion pedagogy question, and David's human passes on the case study.
...
- ~~**No ESLint or Prettier config.**~~ **ESLint adopted 2026-07-16**, the session after the churn hold, with David's approval on the recommended shape. Flagged 2026-06-18 as the expected baseline for a React portfolio repo. **Churn quantified 2026-07-16** (read-only preview, ESLint 10.7 + react-hooks 7.1 stock Vite template config, 80 files): 47 findings in 20 files, but 31 are the two new React-Compiler-era rules (`set-state-in-effect`, `refs`) that collide with documented Cadence patterns (the DemoArea `prevKeyRef`, effect-driven token sync); 11 are the conventional context-file `react-refresh` warnings; the substantive residue is 2 dead-code errors (`TokenLab/index.jsx`: unused `useMotionTokens` import at line 9, unused `_` at line 1223) and 3 `exhaustive-deps` warnings worth individual eyeballs (two PrincipleCard `useLayoutEffect`s, one RailDrawer ref-in-cleanup). The adoption review re-ran the preview against HEAD (numbers unchanged) and judged the three `exhaustive-deps` sites individually: all deliberate (the PrincipleCard effects are transition-triggered by design; the RailDrawer cleanup wants the ref at close time). Shipped shape, one amendment from the review: the template config (`eslint.config.mjs`), the two v7 rules **off** rather than warn (31 standing warnings would drown real signal; the config comment points at CLAUDE.md's patterns), `src/context/` exempted from `react-refresh`, the two dead-code errors fixed, four inline disables with reasons (the three `exhaustive-deps` sites plus the grid's `RIV_PREFETCH_MANIFEST` export), and a `lint` script. `npm run lint` exits clean; 104 tests pass; Token Lab verified on built output. Prettier remains unadopted, undiscussed.
...
- ~~**`docs/claude-workflow.md`** is a stub ("_To be written._") that other docs cite.~~ Written 2026-07-16: the full document (the two surfaces, the loop, what stays human, what Claude Code carries), keeping the verification standing-rule section. The two `docs/browser-project/` citations that pointed at "`claude-workflow.md` at the repo root" (a path retired by the 2026-07-14 archive move) now point at `docs/claude-workflow.md`.
