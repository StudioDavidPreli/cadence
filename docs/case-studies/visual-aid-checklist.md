...
- [x] **V01. Overview video.** DONE 2026-08-05: Vimeo embed (video 1218553606) in a responsive 16:9 frame atop `hosted/index.md` and the single-page doc; the OG-image placeholder retired. NOTE: the video's Vimeo privacy settings refuse playback on localhost ("cannot be played here"), consistent with a domain whitelist; it must list davidpreli.com (and any preview origin David wants) or the hosted page shows Vimeo's privacy card. Verify on the real host at publish. Rig scenes built for it: `?capture=hierarchy-of-motion` and `?capture=solid-drawing` (2026-08-05), and `?capture=export-formats` (2026-08-10, the four export formats scrambling into each other), and `?capture=rive-embed` (2026-08-11, the React-to-Rive boundary as code beside the canvas it renders, the theme flip rebinding the view model instance), and `?capture=presets-explore` (2026-08-11, three token-family panels retiming together across four beats, ending on an Explore state no preset could hold; unlabelled in frame, names belong to the edit). Three of these are composed as **plates**, not full frames: export-formats at 864 × 978 for the green area of the 3D composite's scene map, rive-embed and presets-explore both at 1728 × 864 centered in 1920 × 1080 for the red area of `preRenders/riveBlockReference.png`. Both 1728 × 864 plates record against a flat key color, because their container is semi-transparent over an animated background and a screen recording carries no alpha; the tint is drawn in the composite, not in the rig. Scripts written 2026-08-14 against David's cut: a card take (eighteen on-screen cards) and a voiceover take (152 words, ~2 wps) in `v01-script.md`, each with one first-person beat at the close, plus a fully product-voice VO (David's revision, 145 words, no first person) in `v01-script-product.md`. All three timecoded to the edit.
