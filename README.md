...
### Motion Tiles
A pooled mosaic of Rive tiles that runs the system's three motion personalities at field scale. Snappy, Standard, and Cinematic name the same characters in both tools, and each tool speaks them in its own terms: Token Lab as interaction tokens, the grid as an ambient clock (period, envelope, spread, cell, gap). Change a preset and every tile retimes together. Drag the spread and the change crosses the grid in a wave.
